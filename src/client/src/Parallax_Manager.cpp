...
ParallaxManager::ParallaxManager(const int screenWidth, const int screenHeight)
...
void ParallaxManager::update(const float deltaTime, const float speed)
{
    for (auto& layer : layers) {
        layer.offsetX -= layer.scrollSpeed * speed * deltaTime;
        
        float scaleY      = static_cast<float>(screenHeight) / layer.texture.height;
...
void ParallaxManager::draw()
{
    for (const auto& layer : layers) {
        const float scaleY = static_cast<float>(screenHeight) / layer.texture.height;
        float scaleX       = scaleY;
